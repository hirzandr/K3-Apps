# Modified from : https://github.com/terraform-aws-modules/terraform-aws-rds-aurora/blob/master/main.tf

data "aws_partition" "current" {}

locals {
  create = var.create && var.var_confirm_oke
...
resource "aws_rds_cluster" "this" {
  ...
  # Issue : https://github.com/hashicorp/terraform/issues/16724
  availability_zones = [
    "${format("%sa", "${var.db_region}")}",
    "${format("%sb", "${var.db_region}")}",
    "${format("%sc", "${var.db_region}")}"
  ]
